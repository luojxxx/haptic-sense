...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    // If newline, read usb serial data and execute commands
    if (inChar == '\n') {
      String command_type = getValue(inString, ',', 0);
      String command_val = getValue(inString, ',', 1);

      if (command_type == "vibrate") {
        drv.setRealtimeValue(waveform[command_val.toInt()]);
        Serial.println("Running");
      }

      // Clears string for next command
      inString = "";
    }
  }
}
